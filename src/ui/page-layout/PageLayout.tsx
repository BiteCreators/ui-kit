    ...
    <div className={cn("min-h-[100vh]")}>
      ...
      <ScrollArea className={"h-[calc(100vh-60px)] flex flex-col"}>
        <div
          className={cn(
            "grow",
            !!sidebar && "md:grid md:grid-cols-[220px_1fr] md:mb-0 mb-16",
          )}
        >
          {!!sidebar && (
            <div className={"sticky top-0 left-0 self-start md:block hidden"}>
              {sidebar}
            </div>
          )}
      ...
      {!!mobileMenu && (
        <div className={"md:hidden block absolute bottom-0 w-[100vw] z-40"}>
          {mobileMenu}
        </div>
      )}
    </div>
